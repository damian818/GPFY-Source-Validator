        ...
        if (type === "vendors") {
          ...
          if (!match) {
            crossCheckErrors.push({
              ...
              column: rule.field,
              field: rule.field,
              message: rule.field + " value '" + val + "' not found in Vendors reference data",
          ...
          if (!matches || matches.length === 0) {
            crossCheckErrors.push({
              ...
              column: rule.field,
              field: rule.field,
              message: rule.field + " value '" + val + "' not found in COA reference data",
              solution: "Use a valid ID from the COA reference data",
              actualValue: val,
              isCrossCheck: true,
              type: "error",
            });
          } else if (rule.crossCheck!.coaType && rule.crossCheck!.coaType !== "none") {
             const expectedType = rule.crossCheck!.coaType.toLowerCase();
             const hasCorrectType = matches.some((match: any) => {
               const actualType = match.g_coa_type || match.g_type || match.type || match.coa_type || "";
    ...
    switch (fileType) {
        ...
        data.forEach((row, rowIndex) => {
          ...
          if (vendorId && !vendorMap[vendorId]) {
            crossCheckErrors.push({
              ...
              column: "g_vendor_id",
              field: "g_vendor_id",
              message: "Vendor ID " + vendorId + " not found in reference data",
          ...
          if (vendorId && vendorMap[vendorId] && vendorName && vendorMap[vendorId].g_name !== vendorName) {
            crossCheckErrors.push({
              row: rowNum,
              column: "g_name",
              field: "g_name",
              message: 'Vendor name "' + vendorName + '" doesn\'t match reference name "' + vendorMap[vendorId].g_name + '"',
          ...
          if (currencyId && !coaMap[currencyId]) {
            crossCheckErrors.push({
              ...
              field: "g_currency_id",
              message: "Currency ID " + currencyId + " not found in COA reference data",
          ...
          ["g_subsidiary_debit_id", "g_glaccount_debit_id", "g_department_debit_id"].forEach((field) => {
            ...
            if (value && !coaMap[value]) {
              crossCheckErrors.push({
                ...
                column: field,
                field: field,
                message: field + " value '" + value + "' not found in COA reference data",
                solution: "Use a valid ID from the COA reference data",
                actualValue: value,
                isCrossCheck: true,
                type: "error",
      ...
      case "vendors":
        data.forEach((row, rowIndex) => {
          ...
          if (subsidiaryId && Object.keys(coaMap).length > 0 && !coaMap[subsidiaryId]) {
            crossCheckErrors.push({
              row: rowNum,
          ...
          if (currencyId && Object.keys(coaMap).length > 0 && !coaMap[currencyId]) {
            crossCheckErrors.push({
              row: rowNum,
              column: "g_currency_id",
              field: "g_currency_id",
