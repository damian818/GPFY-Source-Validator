...
function isValidDate(dateString: string, fieldName: string) {
  ...
  if (fieldName.toLowerCase() === "g_period" || fieldName.toLowerCase() === "period") {
    regex = /^\\d{4}-\\d{2}-01$/;
  ...
  } else if (fieldName.toLowerCase() === "invoice_date_created" || fieldName.toLowerCase() === "g_invoice_date") { // Assuming invoice_date_created maps to g_invoice_date or similar if needed. It could be any.
...
export function validateData(
  ...
  rules.forEach((rule) => {
    ...
    if (!fieldExists) {
      ...
      } else if (rule.required === "Insights" || rule.required === "Unapproved/Late") {
  ...
  if (missingRequiredColumns.length > 0) {
    errors.push({
      ...
      solution: "Please add the missing columns to your CSV file",
      type: "error",
    });
  }
  ...
  if (missingWarningColumns.length > 0) {
    warnings.push({
      ...
      message: "Missing recommended columns: " + missingWarningColumns.join(", "),
  ...
  for (let i = 0; i < csvData.length; i++) {
    ...
    const duplicateCheckField = fileType === "coa" ? "g_unique_key" : "g_source_system_id";
    ...
    rules.forEach((rule) => {
      ...
      switch (rule.dataType) {
        case "string":
          if (isEmpty) {
            if (rule.required === "Yes") {
              ...
              warnings.push({
                ...
                solution: "Gappify may still work, but adding it is recommended",
                actualValue: value,
                type: "warning",
              });
            }
            ...
            errors.push({
              ...
              solution: "Shorten the text to " + rule.maxLength + " characters or less",
              actualValue: value,
              type: "error",
            });
          }

          if (rule.allowedValues && rule.allowedValues.length > 0 && value && !rule.allowedValues.includes(String(value))) {
            errors.push({
              row: rowNum,
              column: rule.field,
              field: rule.field,
              message: "Invalid value. Must be one of: " + rule.allowedValues.join(", "),
              solution: "Use an allowed value: " + rule.allowedValues.join(", "),
              actualValue: value,
              type: "error",
            });
          }

          if (rule.field.toLowerCase() === "g_email" && value && !isValidEmail(value)) {
            errors.push({
              row: rowNum,
              column: rule.field,
              field: rule.field,
            ...
            for (const email of emails) {
              if (email.trim() && !isValidEmail(email.trim())) {
                errors.push({
                  ...
                  solution: "Use valid email formats separated by semicolons or commas",
                  actualValue: value,
                  type: "error",
                });
                break;
          ...
          if (["g_currency", "g_currency_name", "g_from_currency", "g_to_currency"].includes(rule.field.toLowerCase()) && value) {
        ...
        case "int":
          ...
          } else if (value !== undefined && value !== null && value !== "" && !isValidInt(value, rule.maxLength || 11)) {
            errors.push({
              row: rowNum,
              column: rule.field,
              field: rule.field,
        ...
        case "decimal":
          ...
          } else if (value !== undefined && value !== null && value !== "" && !isValidDecimal(value, rule.precision || 16, rule.scale || 2)) {
            errors.push({
              row: rowNum,
              column: rule.field,
              field: rule.field,
        ...
        case "date":
          ...
          } else if (value !== undefined && value !== null && value !== "" && !isValidDate(value, rule.field)) {
            let helpText = "Use YYYY-MM-DD format";
            if (rule.field.toLowerCase() === "g_period" || rule.field.toLowerCase() === "period") helpText = "Use YYYY-MM-01 format";
            if (rule.field.toLowerCase() === "invoice_date_created" || rule.field.toLowerCase() === "g_invoice_date") helpText = "Use YYYY-MM-DD or YYYY-MM-DD hh:mm:ss format";
            errors.push({
              row: rowNum,
              column: rule.field,
              field: rule.field,
          ...
          if (isEmpty) {
            if (rule.required === "Yes") {
              ...
              warnings.push({
                ...
                solution: "Adding a boolean value (0, 1, TRUE, FALSE) is recommended",
                actualValue: value,
                type: "warning",
              });
            }
